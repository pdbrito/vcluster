...
func mergeValues(proClient procli.Client, options *Options, log log.Logger) (string, error) {
	...
	chartValues, err := config2.GetExtraValues(chartOptions, logger)
	if err != nil {
		return "", err
	}
...
func toChartOptions(proClient procli.Client, options *Options, log log.Logger) (*config2.ExtraValuesOptions, error) {
	...
	version := config2.KubernetesVersion{}
	if options.KubernetesVersion != "" {
		...
		parsedVersion, err := config2.ParseKubernetesVersionInfo(majorMinorVer)
		if err != nil {
			return nil, err
		}
	...
	return &config2.ExtraValuesOptions{
