...
func main() {
	...
	for _, image := range config2.K3SVersionMap {
	...
	for _, image := range config2.K0SVersionMap {
		if contains(images, image) {
			continue
		}
	...
	for _, image := range config2.K8SAPIVersionMap {
		if contains(images, image) {
			continue
		}
	...
	for _, image := range config2.K8SControllerVersionMap {
		if contains(images, image) {
			continue
		}
	...
	for _, image := range config2.K8SEtcdVersionMap {
		if contains(images, image) {
			continue
		}
